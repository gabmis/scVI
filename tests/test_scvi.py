...
def test_retina():
    run_benchmarks(
        "retina", n_epochs=1, show_batch_mixing=False, save_path="tests/data/"
    )
...
def test_brain_small():
    run_benchmarks(
        "brain_small", n_epochs=1, show_batch_mixing=False, save_path="tests/data/"
    )


def test_hemato():
    run_benchmarks(
        "hemato", n_epochs=1, show_batch_mixing=False, save_path="tests/data/HEMATO/"
    )


def test_pbmc():
    run_benchmarks(
        "pbmc", n_epochs=1, show_batch_mixing=False, save_path="tests/data/PBMC/"
    )


def test_loom():
    run_benchmarks(
        dataset_name="retina.loom",
        n_epochs=1,
