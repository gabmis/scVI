...
def test_cortex():
    ...
    trainer_cortex_vae = UnsupervisedTrainer(
        vae, cortex_dataset, train_size=0.9, use_cuda=use_cuda
    )
    ...
    trainer_cortex_svaec = JointSemiSupervisedTrainer(
        svaec, cortex_dataset, n_labelled_samples_per_class=50, use_cuda=use_cuda
    )
...
def test_synthetic_1():
    ...
    trainer_synthetic_svaec.labelled_set.show_t_sne(
        n_samples=50, color_by="batches and labels"
    )
...
def test_fish_rna():
    ...
    gene_dataset_seq = CortexDataset(
        genes_fish=gene_dataset_fish.gene_names, genes_to_keep=[], additional_genes=50
    )
...
def test_pbmc():
    ...
    purified_t_cells = PurifiedPBMCDataset(
        save_path="tests/data/", filter_cell_types=range(6)
    )  # only t-cells
