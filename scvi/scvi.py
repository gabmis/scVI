...
class VAE(nn.Module):
    def __init__(
        self, nb_genes=100, h_dim=128, latent_dim=5, n_layers=1, keep_prob=0.9
    ):
        ...
        self.first_layer = nn.Sequential(
            nn.Dropout(p=self.dropout_layer_prob), nn.Linear(nb_genes, h_dim), nn.ReLU()
        )
        ...
        self.z_logvar_encoder = nn.Sequential(
            self.first_layer,
            nn.Sequential(self.hidden_layers),
            nn.Linear(h_dim, latent_dim),
        )
        ...
        self.l_mean_encoder = nn.Sequential(self.l_encoder_initial, nn.Linear(h_dim, 1))
        self.l_logvar_encoder = nn.Sequential(
            self.l_encoder_initial, nn.Linear(h_dim, 1)
        )
        ...
        self.decoder_first_layer = nn.Sequential(
            nn.Linear(latent_dim, h_dim), nn.ReLU()
        )
