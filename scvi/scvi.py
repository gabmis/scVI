...
class VAE(nn.Module):
    def __init__(
        ...
        self.l_mean_encoder = nn.Sequential(
            self.l_encoder_initial, nn.Linear(n_hidden, 1)
        )
        self.l_var_encoder = nn.Sequential(
            self.l_encoder_initial, nn.Linear(n_hidden, 1)
        )
        ...
        self.tmp_decoder = nn.Sequential(
    ...
    def reparameterize(self, mu, var):
        ...
        eps = Variable(
            torch.from_numpy(eps).type(torch.FloatTensor), requires_grad=False
        )
    ...
    def sample(self, z):
        return self.px_scale_decoder(
            z
        )  # , self.px_rate_decoder(z), self.px_dropout_decoder(z)
