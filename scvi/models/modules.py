...
class OneHotFCLayers(nn.Module):
    def __init__(self, n_in, n_out, n_cat, n_hidden=128, n_layers=1, dropout_rate=0.1):
        ...
        layers_dim = [n_in] + (n_layers - 1) * [n_hidden] + [n_out]
        self.n_cat = n_cat
        self.fc_layers = FCLayers._sequential(
            layers_dim, n_cat=n_cat, dropout_rate=dropout_rate
        )
...
class ManyOneHotFCLayers(nn.Module):
    def __init__(
        self, n_in, n_out, n_cat_list, n_hidden=128, n_layers=1, dropout_rate=0.1
    ):
        super(ManyOneHotFCLayers, self).__init__()
        layers_dim = [n_in] + (n_layers - 1) * [n_hidden] + [n_out]
        self.n_cat_list = n_cat_list
        self.fc_layers = FCLayers._sequential(
            layers_dim, n_cat=sum(n_cat_list), dropout_rate=dropout_rate
        )
...
class Encoder(nn.Module):
    def __init__(
        self, n_input, n_hidden=128, n_latent=10, n_cat=0, n_layers=1, dropout_rate=0.1
    ):
        super(Encoder, self).__init__()
        self.encoder = FCLayers.create(
            n_in=n_input,
            n_out=n_hidden,
            n_cat=n_cat,
            n_layers=n_layers,
...
class DecoderSCVI(nn.Module):
    def __init__(
        ...
        self.px_decoder = FCLayers.create(
            n_in=n_latent,
            n_out=n_hidden,
            n_layers=n_layers,
            n_hidden=n_hidden,
            dropout_rate=dropout_rate,
            n_cat=[n_batch, n_labels],
        )
    ...
    def forward(self, dispersion, z, library, batch_index=None, y=None):
        ...
        px_rate = (
            torch.exp(torch.clamp(library, 0, 8)) * px_scale
        )  # Total UMI counts clamped to exp(8) ~ 3000
...
class Decoder(nn.Module):
    def __init__(
        self, n_latent, n_output, n_cat=0, n_hidden=128, n_layers=1, dropout_rate=0.1
    ):
        super(Decoder, self).__init__()
        self.decoder = FCLayers.create(
            n_in=n_latent,
            n_out=n_hidden,
            n_cat=n_cat,
            n_layers=n_layers,
