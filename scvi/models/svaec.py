...
class SVAEC(VAE):
    ...
    def __init__(
        self, n_input, n_labels=7, y_prior=None, n_latent=10, n_layers=3, **kwargs
    ):
        super(SVAEC, self).__init__(
            n_input, n_latent=n_latent, n_layers=n_layers, **kwargs
        )
        ...
        self.classifier = Classifier(
            n_latent, self.n_hidden, self.n_labels, self.n_layers, self.dropout_rate
        )
        self.encoder_z2_z1 = Encoder(
            n_input=n_latent + self.n_labels, n_latent=n_latent, n_layers=n_layers
        )
        self.decoder_z1_z2 = Decoder(
            n_latent + self.n_labels, n_latent, n_layers=n_layers
        )
