...
class SVAEC(nn.Module):
    ...
    def forward(self, x, local_l_mean, local_l_var, batch_index=None, y=None):
        ...
        ys, xs, batch_index, local_l_var, local_l_mean, qz1_m, qz1_v, z1 = broadcast_labels(
            ys,
            xs,
            batch_index,
            local_l_var,
        ...
        loss_z1 = (
            -Normal(pz1_m, torch.sqrt(pz1_v)).log_prob(z1)
            + Normal(qz1_m, torch.sqrt(qz1_v)).log_prob(z1)
        ).sum(dim=1)
        kl_divergence_l = kl(
