...
class VAEC(nn.Module):
    def __init__(
        ...
        self.y_prior = (
            y_prior if y_prior is not None else (1 / n_labels) * torch.ones(n_labels)
        )
    ...
    def forward(self, x, local_l_mean, local_l_var, batch_index=None, y=None):
        ...
        ys, xs, batch_index, local_l_var, local_l_mean = broadcast_labels(
        ...
        if self.reconstruction_loss == "zinb":
            reconst_loss = -log_zinb_positive(
                xs, px_rate, torch.exp(self.px_r), px_dropout
            )
