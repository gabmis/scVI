...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        for i_batch, (tensors_train) in enumerate(data_loader_train):
            ...
            reconst_loss_train, kl_divergence_train = vae(
                sample_batch_train,
                local_l_mean_train,
                local_l_var_train,
            ...
            train_loss = torch.mean(
                reconst_loss_train + kl_ponderation * kl_divergence_train
            )
...
@enable_grad()
def train_semi_supervised_jointly(
    vae,
    data_loader_all,
    ...
    for epoch in range(n_epochs):
        ...
        total_train_loss = 0
        for i_batch, (tensors, tensor_classification) in enumerate(
            zip(data_loader_all, cycle(data_loader_labelled))
        ):
            with torch.no_grad():
                if vae.use_cuda:
            ...
            reconst_loss, kl_divergence = vae(
                sample_batch, local_l_mean, local_l_var, batch_index=batch_index, y=None
            )
            ...
            if hasattr(vae, "classify"):
                ...
                classification_loss = F.cross_entropy(
                    vae.classify(sample_batch_cl), labels_cl.view(-1)
                )
...
@enable_grad()
def train_semi_supervised_alternately(
    vae,
    data_loader_all,
    data_loader_labelled,
    data_loader_unlabelled,
...
def train_classifier(
    vae, classifier, *data_loaders, n_epochs=250, lr=0.1, benchmark=False
):
