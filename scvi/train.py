...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        for i_batch, (tensors_train, tensors_test) in enumerate(
            ...
            reconst_loss_mean_train = reconstruction_ratio * torch.mean(
                reconst_loss_train
            )
            kl_divergence_mean_train = torch.mean(kl_divergence_train)
            reconst_loss_mean_test = reconstruction_ratio * torch.mean(
                reconst_loss_test
            )
            kl_divergence_mean_test = torch.mean(kl_divergence_test)

            train_loss = (
                reconst_loss_mean_test
                + reconst_loss_mean_train
