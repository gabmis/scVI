...
def train_semi_supervised(
    ...
    for epoch in range(n_epochs):
        ...
        total_train_loss = 0
        for i_batch, (tensors_train, tensors_test) in enumerate(
            zip(data_loader_train, cycle(data_loader_test))
        ):
            with torch.no_grad():
                if vae.use_cuda:
                    tensors_train = to_cuda(tensors_train)
            ...
            sample_batch_test, local_l_mean_test, local_l_var_test, batch_index_test, _ = (
                tensors_test
            )
            sample_batch_train = sample_batch_train.type(torch.float32)
            ...
            reconst_loss_test, kl_divergence_test = vae(
                sample_batch_test,
                local_l_mean_test,
                local_l_var_test,
            ...
            test_loss = torch.mean(
                reconst_loss_test + kl_ponderation * kl_divergence_test
            )
...
def train_classifier(
    vae, classifier, data_loader_train, data_loader_test, n_epochs=250, lr=0.1
):
