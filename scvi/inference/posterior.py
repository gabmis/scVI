...
class Posterior:
    ...
    def __init__(
        ...
        self.data_loader_kwargs.update(
            {"collate_fn": gene_dataset.collate_fn, "sampler": sampler}
        )
    ...
    def imputation_benchmark(self, n_samples=8, verbose=False):
        ...
        for tensors, corrupted_tensors in zip(
            self.sequential(batch_size=batch_size),
...
def entropy_batch_mixing(
    ...
    keep_idx = np.random.choice(
        np.arange(n_samples), size=min(len(latent_space), max_number), replace=False
    )
...
def de_cortex(
    px_scale, all_labels, gene_names, M_permutation=100000, permutation=False
):
    """
    Output average over statistics in a symmetric way (a against b)
    forget the sets if permutation is True
    ...
    result = [
        (gene_name, res[np.where(gene_names == gene_name)[0]][0])
