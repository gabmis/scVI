...
class Posterior:
    ...
    def one_vs_all_degenes(
        ...
        if cell_labels is not None:
            if len(cell_labels) != len(self.gene_dataset):
                raise ValueError(
                    " the length of cell_labels have to be the same as the number of cells"
                )
    ...
    def within_cluster_degenes(
        ...
        for i, x in enumerate(cluster_id):
            if subset is None:
                idx1 = (cell_labels == i) * (states == True)
                idx2 = (cell_labels == i) * (states == False)
            else:
                idx1 = (cell_labels == i) * subset * (states == True)
                idx2 = (cell_labels == i) * subset * (states == False)
            if np.sum(idx1) > min_cells and np.sum(idx2) > min_cells:
                de_cluster.append(x)
                res = self.differential_expression_score(
                    idx1=idx1,
    ...
    def get_stats(self, verbose=True):
        ...
        for tensors in self.sequential(batch_size=128):
            ...
            px_scale, px_r, px_rate, px_dropout, qz_m, qz_v, z, ql_m, ql_v, library = self.model.inference(
                x, batch_index, y
            )
