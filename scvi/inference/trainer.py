...
class Trainer:
    ...
    @torch.no_grad()
    def get_all_latent_and_imputed_values(
        self,
        save_imputed=False,
        filename_imputation="imputed_values",
        save_shape_genes_by_cells=False,
        save_latent=False,
        ...
        for tensors in all_dataset:
            ...
            ret["scaled_imputed_values"] += [
                self.model.get_sample_scale(sample_batch, batch_index=batch_index).cpu()
            ]
        for key in ret.keys():
            if len(ret[key]) > 0:
