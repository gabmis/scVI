...
from scvi.metrics.clustering import (
    get_latent,
...
class VariationalInference(Inference):
    ...
    def marginal_ll(self, name, verbose=False, n_mc_samples=1000):
        ll = compute_marginal_log_likelihood(
            self.model, self.data_loaders[name], n_mc_samples
        )
        if verbose:
    ...
    def imputation(
        ...
        original_list, imputed_list = imputation(
            self,
            name,
            rate=rate,
            n_epochs=n_epochs,
        ...
        median_imputation_score = np.median(
        ...
        if verbose:
            print(
                "Imputation Scores [corruption:%s - rate:%.2f] on  %s after %i:"
                ...
                % (
                    ...
                    name,
                    n_epochs,
                    median_imputation_score,
                    mean_imputation_score,
                )
            )

        plot_imputation(
            np.concatenate(original_list), np.concatenate(imputed_list), title=title
        )
    ...
    def nn_overlap_score(self, name="sequential", verbose=True, **kwargs):
        if hasattr(self.gene_dataset, "adt_expression_clr"):
            assert (
                name == "sequential"
            )  # only works for the sequential data_loader (mapping indices)
    ...
    def entropy_batch_mixing(self, name, verbose=False, **kwargs):
        if self.gene_dataset.n_batches == 2:
            latent, batch_indices, labels = get_latent(
                self.model, self.data_loaders[name]
            )
...
class SemiSupervisedVariationalInference(VariationalInference):
    ...
    def unsupervised_accuracy(self, name, verbose=False):
        uca = unsupervised_classification_accuracy(self.model, self.data_loaders[name])[
            0
        ]
        if verbose:
    ...
    def svc_rf(self, **kwargs):
        ...
        svc_scores, _ = compute_accuracy_svc(
            data_train, labels_train, data_test, labels_test, **kwargs
        )
        rf_scores, _ = compute_accuracy_rf(
            data_train, labels_train, data_test, labels_test, **kwargs
        )
...
class AlternateSemiSupervisedVariationalInference(SemiSupervisedVariationalInference):
    ...
    def on_epoch_end(self):
        self.classifier_inference.train(
...
class VariationalInferenceFish(VariationalInference):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(VariationalInferenceFish, self).__init__(
        ...
        self.data_loaders = TrainTestDataLoadersFish(
            gene_dataset_seq,
            gene_dataset_fish,
    ...
    def ll(self, name, verbose=False):
        ...
        for i_batch, tensors in enumerate(data_loader):
            if name == "train_fish" or name == "test_fish":
                sample_batch, local_l_mean, local_l_var, batch_index, labels, _, _ = (
                    tensors
                )
                reconst_loss, kl_divergence = self.model(
                    sample_batch,
        ...
        n_samples = (
            ...
            if not (
                hasattr(data_loader, "sampler")
                and hasattr(data_loader.sampler, "indices")
            )
