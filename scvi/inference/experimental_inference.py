...
def mmd_fourier(x1, x2, bandwidth=2.0, dim_r=500):
    d = x1.size(1)
    rw_n = sqrt(2.0 / bandwidth) * Normal(0.0, 1.0 / sqrt(d)).sample((dim_r, d)).type(
        x1.type()
    )
    rb_u = 2 * pi * Uniform(0.0, 1.0).sample((dim_r,)).type(x1.type())
    rf0 = sqrt(2.0 / dim_r) * torch.cos(F.linear(x1, rw_n, rb_u))
    rf1 = sqrt(2.0 / dim_r) * torch.cos(F.linear(x2, rw_n, rb_u))
...
def mmd_objective(z, batch_index, n_batch):
    ...
    for j, z_j in enumerate(z_part):
        z0_ = z_j
        aux_z0 = Normal(0.0, 1.0).sample((1, z_dim)).type(z0_.type())
        z0 = torch.cat((z0_, aux_z0), dim=0)
        if len(z_part) == 2:
            z1_ = z_part[j + 1]
            aux_z1 = Normal(0.0, 1.0).sample((1, z_dim)).type(z1_.type())
...
def mmd_loss(self, tensors, *next_tensors):
    if self.epoch > self.warm_up:  # Leave a warm-up
        ...
        qm_z, _, _ = self.model.z_encoder(
            torch.log(1 + sample_batch), label
        )  # label only used in VAEC
...
def adversarial_loss(self, tensors, *next_tensors):
    if self.epoch > self.warm_up:
        ...
        cls_loss = self.scale * F.cross_entropy(
            self.adversarial_cls(qm_z), batch_index.view(-1)
        )
...
def adversarial_loss_fish(self, tensors_seq, tensors_fish):
    if self.epoch > self.warm_up:
        ...
        cls_loss = self.scale * F.cross_entropy(
            self.adversarial_cls(z), torch.zeros_like(batch_index).view(-1)
        )
        ...
        cls_loss += self.scale * F.cross_entropy(
            self.adversarial_cls(z), torch.ones_like(batch_index).view(-1)
        )
        self.optimizer_cls.zero_grad()
        cls_loss.backward(retain_graph=True)
        self.optimizer_cls.step()
...
def adversarial_train(self, n_epochs=20, lr=1e-3, weight_decay=1e-4):
    self.adversarial_cls = Classifier(
        self.model.n_latent, n_labels=self.model.n_batch, n_layers=3
    )
    if self.use_cuda:
        self.adversarial_cls.cuda()
    self.optimizer_cls = torch.optim.Adam(
        filter(lambda p: p.requires_grad, self.adversarial_cls.parameters()),
        lr=lr,
