...
class ClassifierTrainer(Trainer):
    ...
    def __init__(self, *args, sampling_model=None, use_cuda=True, **kwargs):
        ...
        self.train_set, self.test_set = self.train_test(
            self.model, self.gene_dataset, type_class=AnnotationPosterior
        )
    ...
    def loss(self, tensors_labelled):
        ...
        x = (
            self.sampling_model.z_encoder(torch.log(1 + x))[0]
            if self.sampling_model is not None
            else x
        )
...
class SemiSupervisedTrainer(UnsupervisedTrainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.full_dataset = self.create_posterior(
            shuffle=True, type_class=AnnotationPosterior
        )
        self.labelled_set = self.create_posterior(
            indices=indices_labelled, type_class=AnnotationPosterior
        )
        self.unlabelled_set = self.create_posterior(
            indices=indices_unlabelled, type_class=AnnotationPosterior
        )
