...
def run_benchmarks(
    ...
    if isinstance(vae, VAE):
        best_ll = adapt_encoder(
            vae, data_loader_test, n_path=1, n_epochs=1, record_freq=1
        )
...
def run_benchmarks_classification(
    ...
    cls = Classifier(
        n_input=n_latent, n_labels=gene_dataset.n_labels, n_layers=3, use_cuda=use_cuda
    )
