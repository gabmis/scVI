...
def cortex_benchmark(n_epochs=250, use_cuda=True, save_path="data/", show_plot=True):
    ...
    trainer_cortex_vae.train_set.imputation_benchmark(
        verbose=(n_epochs > 1), save_path=save_path, show_plot=show_plot
    )
...
def benchmark_fish_scrna(gene_dataset_seq, gene_dataset_fish):
    ...
    concatenated_matrix = np.concatenate(
        (
            gene_dataset_fish.X[:, vae.indexes_to_keep],
            gene_dataset_seq.X[:, vae.indexes_to_keep],
        )
    )
    ...
    _, _, = compute_accuracy_nn(
        pca_latent_seq, pca_labels_seq.ravel(), pca_latent_fish, pca_labels_fish.ravel()
    )
