...
class PbmcDataset(GeneExpressionDataset):
    ...
    def __init__(self, save_path="data/PBMC/"):
        ...
        self.gene_names_filename = (
            "michael_gene_names.csv"
        )  # from romain, manually put into PBMC folder
    ...
    def preprocess(self):
        ...
        gene_names = np.array(
            pd.read_csv(self.save_path + self.gene_names_filename, index_col=0)["x"],
            dtype=str,
        )
        expression = pd.DataFrame(
            io.mmread(self.save_path + self.count_filename).T.A, columns=gene_names
        )
