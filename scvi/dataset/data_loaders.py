...
class DataLoaders:
    ...
    def __call__(self, shuffle=False, indices=None):
        ...
        return DataLoaderWrapper(
            self.gene_dataset, use_cuda=self.use_cuda, sampler=sampler, **self.kwargs
        )
...
class TrainTestDataLoaders(DataLoaders):
    ...
    def __init__(self, gene_dataset, train_size=0.1, test_size=None, seed=0, **kwargs):
        ...
        self.dict.update({"train": data_loader_train, "test": data_loader_test})
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(SemiSupervisedDataLoaders, self).__init__(
            gene_dataset, use_cuda=use_cuda, **kwargs
        )
...
class AlternateSemiSupervisedDataLoaders(SemiSupervisedDataLoaders):
    ...
    def classifier_data_loaders(self):
        data_loaders = DataLoaders(
            gene_dataset=self.gene_dataset, use_cuda=self.use_cuda, **self.kwargs
        )
...
class TrainTestDataLoadersFish(DataLoaders):
    ...
    def __init__(
        ...
        **data_loaders_kwargs
        ...
        :param test_size: float, int, or None (default is None)
        """
        seq = TrainTestDataLoaders(
            gene_dataset_seq, train_size, test_size, seed, **data_loaders_kwargs
        )
        fish = TrainTestDataLoaders(
            gene_dataset_fish, train_size, test_size, seed, **data_loaders_kwargs
        )
        self.dict = {}
        self.dict.update(
            {
