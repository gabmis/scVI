...
class CiteSeqDataset(GeneExpressionDataset):
    ...
    def preprocess(self):
        ...
        self.expression = expression = pd.read_csv(
            self.save_path + self.download_name_rna, index_col=0, compression="gzip"
        ).T
        ...
        self.adt_centered = adt_centered = pd.read_csv(
            self.save_path + self.download_name_adt_centered,
            index_col=0,
            compression="gzip",
        )
        self.adt_expression_clr = adt_centered.T.values
        assert (
            self.protein_markers == np.array(adt_centered.index).astype(np.str)
        ).all()
