...
class GeneExpressionDataset(Dataset):
    ...
    def update_cells(self, subset_cells):
        new_n_cells = (
            len(subset_cells)
            if subset_cells.dtype is not np.bool
    ...
    def subsample_genes(self, new_n_genes=None, subset_genes=None):
        ...
        if subset_genes is None and (
            not hasattr(self, "gene_names")
            or new_n_genes is False
            or new_n_genes >= n_genes
        ):
    ...
    def filter_cell_types(self, cell_types):
        ...
        if type(cell_types[0]) is not int:
            ...
            cell_types_idx = np.array(
                [current_cell_types.index(cell_type) for cell_type in cell_types]
            )
        else:
    ...
    def concat_datasets(*gene_datasets, on="gene_names", shared_labels=True):
        ...
        Xs = [
            filter_genes(gene_dataset, gene_names_ref, on=on)[0]
        ...
        _, local_mean, local_var, _, _ = GeneExpressionDataset.get_attributes_from_matrix(
            X
        )
