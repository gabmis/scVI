...
def concat_datasets(*gene_datasets):
    ...
    def filter_genes(gene_dataset, gene_names_ref):
        ...
        idx_genes = np.array(
            [gene_names.index(gene_name) for gene_name in gene_names_ref], dtype=np.byte
        )
        print(len(idx_genes))
        return gene_dataset.X[:, idx_genes]

    gene_names = set.intersection(
        *[set(gene_dataset.gene_names) for gene_dataset in gene_datasets]
    )
...
def get_data_loaders(gene_dataset, n_labelled_samples_per_class, **kwargs_data_loader):
    ...
    indices_labelled, indices_unlabelled = get_indices(
        gene_dataset.labels, [n_labelled_samples_per_class] * gene_dataset.n_labels
    )
    ...
    data_loader_labelled = DataLoader(
        gene_dataset,
        collate_fn=gene_dataset.collate_fn,
        sampler=SubsetRandomSampler(indices_labelled),
        **kwargs_data_loader
    )
    data_loader_unlabelled = DataLoader(
        gene_dataset,
        collate_fn=gene_dataset.collate_fn,
