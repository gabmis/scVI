...
class SyntheticDatasetCorr(GeneExpressionDataset):
    def __init__(
        ...
        self.is_highly_exp = np.zeros(
            (self.n_batches, self.batch_size, self.n_genes_total), dtype=np.bool
        )
        ...
        logging.info(
            "Poisson Params extremal values: ",
            self.exprs_param.min(),
            self.exprs_param.max(),
        )
