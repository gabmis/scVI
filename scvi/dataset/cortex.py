...
class CortexDataset(GeneExpressionDataset):
    ...
    def __init__(
        self, save_path="data/", genes_to_keep=[], genes_fish=[], additional_genes=558
    ):
        # Generating samples according to a ZINB process
        self.save_path = save_path
        self.download_name = "expression.bin"
        self.url = (
            "https://storage.googleapis.com/linnarsson-lab-www-blobs/blobs/cortex/"
    ...
    def preprocess(self):
        ...
        selected = np.std(expression_data, axis=0).argsort()[-self.additional_genes :][
            ::-1
        ]
