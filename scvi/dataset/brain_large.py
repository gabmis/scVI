...
def subsample_individuals(gbm, individuals_indices):
    return GeneBCMatrix(
        gbm.gene_ids, gbm.gene_names, gbm.barcodes, gbm.matrix[individuals_indices, :]
    )
...
class BrainLargeDataset(GeneExpressionDataset):
    def __init__(self, subsample_size=20):
        ...
        super(BrainLargeDataset, self).__init__(
            [np.array(h5_object.matrix.transpose().todense())]
        )
    ...
    def preprocess(self):
        ...
        )  # Warning : 20e3 is a float in python3
        subset = np.sort(
            np.random.choice(
                gene_bc_matrix.barcodes.size, size=subsample_bcs, replace=False
            )
        )
