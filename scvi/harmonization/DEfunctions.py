...
def GetScale(trainer, n_batches, n_samples_per_batch, gene_dataset, selection, seed=0):
    ...
    for batch in range(n_batches):
        idx = np.random.choice(
            np.arange(len(gene_dataset))[selection], n_samples_per_batch
        )
...
def GetScaleFromBatch(
    trainer, batchid, n_samples_per_batch, gene_dataset, selection, seed=0
):
    scale = []
    np.random.seed(seed)
...
def ComputeDE(
    ...
    for i in np.arange(0, nrep):
        scale1 = GetScaleFromBatch(
            trainer, batch1, n_samples_per_batch, gene_dataset, idx1, seed=seeds[i]
        )
        scale2 = GetScaleFromBatch(
            trainer, batch2, n_samples_per_batch, gene_dataset, idx2, seed=seeds[i]
        )
...
def One2AllBayes(trainer, gene_dataset, clusters, batchid, subset=None, mincells=10):
    ...
    for i in cluster_id:
        ...
        if np.sum(idx1) > mincells and np.sum(idx2) > mincells:
            ...
            res = pd.DataFrame(
                [bayes[0, :], bayes[1, :], mean1, mean2, nonz1, nonz2],
                index=["bayes1", "bayes2", "mean1", "mean2", "nonz1", "nonz2"],
                columns=gene_dataset.gene_names,
            ).T
...
def WithinClusterDE(
    trainer, gene_dataset, clusters, group1, group2, batch1, batch2, mincells=10
):
