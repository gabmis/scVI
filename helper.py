...
def train_model(
    ...
    if kl is None:
        warmup = lambda x: np.minimum(1, x / 400.0)
    else:
        warmup = lambda x: kl
    ...
    for t in range(iterep * num_epochs):
        ...
        index_train = np.random.choice(
            np.arange(expression_train.shape[0]), size=batch_size
        )
        x_train = expression_train[index_train].astype(np.float32)

        # prepare data dictionaries
        dic_train = {
            model.expression: x_train,
            model.training_phase: True,
            model.kl_scale: kl,
        }
        dic_test = {
            model.expression: expression_test,
