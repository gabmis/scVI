...
def dense(
    x, num_outputs, STD=0.01, keep_prob=None, activation=None, bn=False, phase=None
):
    """
...
def mmd_fourier(x1, x2, bandwidth=2.0, dim_r=500):
    """
    ...
    rW_n = tf.sqrt(2.0 / bandwidth) * tf.random_normal([d, dim_r]) / np.sqrt(d)
    rb_u = 2 * np.pi * tf.random_uniform([dim_r])
    rf0 = tf.sqrt(2.0 / dim_r) * tf.cos(tf.matmul(x1, rW_n) + rb_u)
    rf1 = tf.sqrt(2.0 / dim_r) * tf.cos(tf.matmul(x2, rW_n) + rb_u)
    result = tf.reduce_sum(
        (tf.reduce_mean(rf0, axis=0) - tf.reduce_mean(rf1, axis=0)) ** 2
    )
    return tf.sqrt(result)


def mmd_rbf(x1, x2, bandwidths=1.0 / (2 * (np.array([1.0, 2.0, 5.0, 8.0, 10]) ** 2))):
    """
    ...
    return tf.sqrt(
        tf.reduce_mean(x1x1) - 2 * tf.reduce_mean(x1x2) + tf.reduce_mean(x2x2)
    )
...
def mmd_objective(z, s, sdim):
    ...
    for j, z_j in enumerate(z_part):
        z0_ = z_j
        aux_z0 = tf.random_normal(
            [1, z_dim]
        )  # if an S category does not have any samples
...
class scVIModel:
    def __init__(
        ...
        mmd_scale=None,
        phase=None,
        library_size_mean=None,
        library_size_var=None,
        apply_mmd=False,
        dispersion="gene",
        n_layers=1,
        n_hidden=128,
        ...
        print(
            "Running scVI on "
            + str(self.expression.get_shape().as_list()[1])
            + " genes"
        )
        ...
        if type(library_size_mean) in [float, np.float64]:
            ...
            if library_size_mean.get_shape().as_list()[0] != num_batches:
                raise ValueError(
                    "provide correct prior for library size (check batch shape)"
                )
            else:
        ...
        print(
            "Will work on mode " + self.library_mode + " for incorporating library size"
        )
        ...
        print(
            "Will work on mode "
            + self.dispersion
            + " for modeling inverse dispersion param"
        )
    ...
    def variational_distribution(self):
        ...
        self.ql_m = dense(
            h, 1, activation=None, bn=False, keep_prob=None, phase=self.training_phase
        )
        self.ql_v = dense(
            h, 1, activation=tf.exp, bn=False, keep_prob=None, phase=self.training_phase
        )
    ...
    def generative_model(self):
        ...
        if self.dispersion == "gene-cell":
            ...
            if self.batch_ind is None:
                ...
                self.px_r = tf.Variable(
                    tf.random_normal([self.num_batches, self.n_input]), name="r"
                )
    ...
    def optimize(self):
        ...
        # VAE loss
        recon = log_zinb_positive(
            self.expression, self.px_rate, local_dispersion, self.px_dropout
        )
        ...
        kl_gauss_l = 0.5 * tf.reduce_sum(
            ...
            + tf.log(1e-8 + local_l_var)
            - tf.log(1e-8 + self.ql_v)
            - 1,
            1,
        )
    ...
    def imputation(self):
        ...
        self.zero_prob = tf.nn.softplus(
            -self.px_dropout
            + tf.exp(self.px_r) * self.px_r
            - tf.exp(self.px_r) * tf.log(tf.exp(self.px_r) + self.px_rate + 1e-8)
